...
function onSection() {
  ...
  if (currentSection.getTool().getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED) {
    ...
    writeBlock("D" + rpmFormat.format(maximumSpindleSpeed), sOutput.format(tool.surfaceSpeed * ((unit == MM) ? 1 / 1000.0 : 1 / 12.0)), mSpindle);
  } else {
    writeBlock(sOutput.format(spindleSpeed), mSpindle);
  }
