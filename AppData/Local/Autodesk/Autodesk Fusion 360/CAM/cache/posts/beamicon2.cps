...
function onOpen() {
  ...
  } else if (getProperty("useRadius") == "incrementalIJK") {
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    writeBlock("T" + toolFormat.format(tool.number), mFormat.format(tool.getManualToolChange() ? 66 : 6));
    if (tool.comment) {
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  case "absoluteIJK":
    ...
    break;
  case "incrementalIJK":
    i = iOutput.format(cx - start.x, 0);
    j = jOutput.format(cy - start.y, 0);
    k = kOutput.format(cz - start.z, 0);
  }
