...
function onOpen() {
  ...
  if (getProperty("rotaryTableAxis") != "none") {
    ...
    if (rotary < 0) {
      error(localize("Valid rotaryTableAxis values are: None, X, Y, -X, -Y"));
      return;
    }
    rotary -= 3;
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    if (!isFirstSection() && !getProperty("useToolChanger")) {
  ...
  // wcs
  /*
  if (!getProperty("useToolChanger") || insertToolCall) { // force work offset when changing tool
...
*/
  ...
  if (insertToolCall || retracted) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      // writeBlock(gMotionModal.format(0), conditional(getProperty("useToolChanger"), gFormat.format(43)), zOutput.format(initialPosition.z));
    } else {
      writeBlock(
        gAbsIncModal.format(90),
        gMotionModal.format(0),
        // conditional(getProperty("useToolChanger"), gFormat.format(43)),
...
function onCyclePoint(x, y, z) {
  ...
  var tapping = getProperty("useTappingCycle") &&
    ((cycleType == "tapping") ||
    (cycleType == "right-tapping") || (cycleType == "left-tapping") ||
    (cycleType == "tapping-with-chip-breaking"));

  if (isFirstCyclePoint() || (tapping && !cycleExpanded)) {
    ...
    case "deep-drilling":
      ...
      break;
    case "tapping":
      if (P > 0 || !getProperty("useTappingCycle")) {
        expandCyclePoint(x, y, z);
      } else {
        gCycleModal.reset();
        feedOutput.reset();
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(gFormat.format(41), conditional(!getProperty("useToolChanger"), "O" + xyzFormat.format(tool.diameter / 2)));
        writeBlock(
      ...
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock(gFormat.format(42), conditional(!getProperty("useToolChanger"), "O" + xyzFormat.format(tool.diameter / 2)));
        writeBlock(
          gMotionModal.format(1),
          x,
          y,
...
function onLinear5D(_x, _y, _z, _a, _b, _c, feed) {
  ...
  if (x || y || z || a || b || c) {
    ...
    if (getNextRecord().isMotion()) { // try not to output feed without motion
      ...
      writeBlock(gFeedModeModal.format(f.mode), gMotionModal.format(1), f.frn);
    }
  }
}
