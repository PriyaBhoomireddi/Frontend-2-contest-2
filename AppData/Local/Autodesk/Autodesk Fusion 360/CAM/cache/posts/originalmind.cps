...
properties = {
  ...
  omitG43Length: {
    title: "Omit G43 tool length offset",
    description: "Enable to omit the G43 tool length offset.",
    type: "boolean",
    value: false,
    scope: "post"
  },
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    if (!isFirstSection() && getProperty("optionalProgramStop")) {
      onCommand(COMMAND_OPTIONAL_STOP);
    }

    if (tool.number > numberOfToolSlots) {
      error(localize("Tool number exceeds maximum value."));
      return;
    }
  ...
  if (workOffset > 0) {
    if (workOffset > 6) {
      ...
      if (workOffset != currentWorkOffset) {
        forceWorkPlane();
        writeBlock(gFormat.format(59) + "." + code); // G59.1->G59.3
