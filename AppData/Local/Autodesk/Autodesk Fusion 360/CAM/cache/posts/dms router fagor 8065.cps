...
properties = {
  ...
  safeZHeight: {
    title: "Safe Z height",
    description: "Safe retract height for operations in G53 mode.",
    type: "number",
    value: 0,
    scope: "post"
  },
  ...
  if (workOffset > 0) {
    if (workOffset > 4) {
      ...
      if (workOffset != currentWorkOffset) {
        gWCSModal.reset();
        writeBlock(gWCSModal.format(159) + "=" + code);
      ...
      if (workOffset != currentWorkOffset) {
        gWCSModal.reset();
        writeBlock(gWCSModal.format((53 + workOffset))); // G54->G59
        currentWorkOffset = workOffset;
        writeln(";(ORGX58=0,ORGY58=0,ORGZ58=0)");
        writeBlock(gFormat.format(58));
      }
...
function onSection() {
  ...
  if ((insertToolCall || forceSmoothing) && getProperty("fiveAxis")) {
    writeBlock("#KIN ID[1]");
    writeBlock("#RTCP ON");
...
*/
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(
        ...
        zOutput.format(initialPosition.z), conditional(!getProperty("fiveAxis"))
      );
    } else {
      writeBlock(
        gAbsIncModal.format(90),
