...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    if (tool.number > 10) {
      error(localize("Tool Number should be between 1 and 10"));
      return;
    } else {
      if (spindle == 1) {
        writeBlock("T" + toolFormat.format(100 + tool.number));
      } else {
        writeBlock("T" + toolFormat.format(200 + tool.number));
      }
    }
    ...
    /*
    if (getProperty("preloadTool")) { // might be M94 Txx
...
*/
  ...
  if (insertToolCall ||
    ...
    if (!tapping || (tapping && !(getProperty("useRigidTapping") == "without"))) {
      writeBlock(
        mFormat.format(getCurrentSpindle() * 10 + (tool.clockwise ? 3 : 4)), sOutput.format(spindleSpeed)
      );
    }
  ...
  */
  ...
  if (insertToolCall || retracted) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(gMotionModal.format(0), gFormat.format(43), zOutput.format(initialPosition.z), hFormat.format(tool.lengthOffset));
    } else {
      writeBlock(
        gAbsIncModal.format(90),
        gMotionModal.format(0),
        ...
        zOutput.format(initialPosition.z), hFormat.format(tool.lengthOffset)
      );
    }
