/**
  ...
  CAMaster with WinCNC control post processor configuration.
...
description = "CAMaster with WinCNC control";
vendor = "CAMaster";
...
longDescription = "CAMaster WinCNC post with support for an automatic tool changer. Enable the 'useToolChanger' property for support of the ATC.  The optionalStop property is typically enabled when an ATC is not available.";
  ...
  */
  ...
  if (insertToolCall || retracted) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(conditional(getProperty("useToolChanger"), gFormat.format(43)));
      writeBlock(gMotionModal.format(0), zOutput.format(initialPosition.z));
    } else {
      writeBlock(
        gAbsIncModal.format(90),
    ...
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(
          ...
          y,
          z,
          conditional(getProperty("useToolChanger"), "O" + xyzFormat.format(tool.diameter / 2)),
          f
        );
        break;
