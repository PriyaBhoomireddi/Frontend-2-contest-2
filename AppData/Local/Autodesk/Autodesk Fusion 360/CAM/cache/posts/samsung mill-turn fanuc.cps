...
function goHome(forceY) {
  ...
  if (getProperty("useG28")) {
    ...
    if (gotYAxis && (yAxisIsEnabled || forceY)) {
      setYAxisMode(true);
      var yAxis = "V" + yFormat.format(getProperty("g53HomePositionY"));
    }
    gMotionModal.reset();
    writeBlock(gMotionModal.format(0), gFormat.format(53), "X" + xFormat.format(getProperty("g53HomePositionX")), yAxis);
    writeBlock(
      gMotionModal.format(0), gFormat.format(53),
      "Z" + zFormat.format((getSpindle(true) == SPINDLE_MAIN) ? getProperty("g53HomePositionZ") : getProperty("g53HomePositionSubZ"))
    );
  }
}
