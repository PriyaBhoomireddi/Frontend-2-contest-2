...
longDescription = "Post for the Mekanika router with PlanetCNC controller.";
extension = "nc";
setCodePage("ascii");
...
function onOpen() {
  ...
  if (getNumberOfSections() > 0) {
    var firstWorkOffset = getSection(0).workOffset;
    for (var i = 0; i < getNumberOfSections(); ++i) {
      if (firstWorkOffset != getSection(i).workOffset) {
