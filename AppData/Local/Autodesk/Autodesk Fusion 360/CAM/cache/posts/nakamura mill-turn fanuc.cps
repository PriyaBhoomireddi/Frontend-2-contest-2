...
function getCode(code, spindle) {
  ...
  case "START_SPINDLE_CW":
    ...
    case SPINDLE_LIVE:
      ...
      return (getMachiningDirection(currentSection) == MACHINING_DIRECTION_RADIAL) ? 89 : (getSpindle(true) == SPINDLE_MAIN) ? 89 : 88;
    }
    break;
  case "START_SPINDLE_CCW":
    ...
    case SPINDLE_LIVE:
      machineState.liveToolIsActive = true;
      return (getMachiningDirection(currentSection) == MACHINING_DIRECTION_RADIAL) ? 88 : (getSpindle(true) == SPINDLE_MAIN) ? 88 : 89;
    }
    break;
    ...
    case "secondary-spindle-return":
      ...
      if (secondaryHome) {
        ...
        if ((transferCodes != undefined) && !transferCodes.cutoff) {
          writeBlock(gFormat.format(getCode("SPINDLE_SYNCHRONIZATION_OFF", getSpindle(true))), formatComment("CANCEL SYNCHRONIZATION"));
        } else {
