...
/**
  ...
  $Revision: 43284 e95941388c338e192dbea5673316b862969756b9 $
  $Date: 2021-04-27 02:45:57 $
...
var useMultiAxisFeatures = true; // false = use rotary axis positions, true = use Euler angles
...
function getMachineType() {
  if (modelType == "D200Z") {
    var bAxis = createAxis({coordinate:1, table:true, axis:[0, 0.71325, -0.700909], range:[0, 180], preference:1});
    ...
    var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-120, 120], preference:0});
    var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:true, preference:0});
...
function onOpen() {
  ...
  if (true) {
    getMachineType(); // default
  } else if (false) { // note: setup your machine here
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    if (!is3D() && machineConfiguration.isMultiAxisConfiguration() && (machineConfiguration.getNumberOfAxes() >= 4)) {
      writeBlock(mFormat.format(10), mFormat.format(12));
    }
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    if (!is3D() && machineConfiguration.isMultiAxisConfiguration() && (machineConfiguration.getNumberOfAxes() >= 4)) {
      writeBlock(mFormat.format(11), mFormat.format(13));  //unlock 4th & 5th-axis motion
    }
    return;
