...
function onOpen() {
  ...
  if (getProperty("rotaryTableAxis") != "none" && getProperty("useTrunnion")) {
    error(localize("You cannot enable a 4-axis rotary table and a 5-axis trunnion at the same time."));
  }
  if (getProperty("useTrunnion")) {
    var bAxis = createAxis({coordinate:1, table:true, axis:[0, 1, 0], range:[-35, 110], preference:1});
    var cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:true});
    machineConfiguration = new MachineConfiguration(bAxis, cAxis);

    setMachineConfiguration(machineConfiguration);
    optimizeMachineAngles2(getProperty("useTCP") ? 0 : 1); // 0 = TCP Mode ON, 1 = TCP Mode OFF
  } else if (getProperty("rotaryTableAxis") != "none") {
  ...
  writeComment(" #### " + String(longDescription) + " #### ");
  writeComment(" #### " + String(vendorUrl) + " #### ");
  writeln(";");
...
function onSection() {
  ...
  if (workOffset > 0) {
    if (workOffset > 6) {
      if (workOffset > 9) {
        ...
        var p = 59 + ((workOffset - 6) / 10);
        if (workOffset != currentWorkOffset) {
          writeBlock(gFormat.format(p)); // G59.p
  ...
  if (insertToolCall || retracted) {
    ...
    if (currentSection.isMultiAxis() && getProperty("useTCP")) {
      writeBlock(mFormat.format(102), hFormat.format(lengthOffset), formatComment("TCP ON"));
    }
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(gMotionModal.format(0), gFormat.format(43), zOutput.format(initialPosition.z),
        conditional((!currentSection.isMultiAxis() || !getProperty("useTCP")), hFormat.format(lengthOffset)));
    } else {
      writeBlock(
        gAbsIncModal.format(90),
        gMotionModal.format(0),
...
function getOptimizedMode() {
  ...
  return !getProperty("useTCP"); // always return false for non-TCP based heads
}
