...
function onOpen() {
  ...
  if ((getProperty("spindleDirectionCode") != 3) &&
      (getProperty("spindleDirectionCode") != 4) &&
      (getProperty("spindleDirectionCode") != 34)) {
    error(localize("The property 'spindleDirectionCode' must be 3, 4, or 34."));
    return;
  }
...
function onSection() {
  ...
  if (insertToolCall ||
    ...
    if (getProperty("spindleDirectionCode") == 34) {
      writeBlock(mFormat.format(tool.clockwise ? 3 : 4));
    } else {
      writeBlock(mFormat.format(getProperty("spindleDirectionCode")));
    }
  }
