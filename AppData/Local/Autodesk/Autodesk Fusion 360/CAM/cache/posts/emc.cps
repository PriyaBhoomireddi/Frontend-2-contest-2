...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    if (getProperty("preloadTool") && getProperty("useToolChanger")) {
  ...
  if (insertToolCall || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(gMotionModal.format(0),
        conditional(getProperty("useToolChanger"), gFormat.format(43)), zOutput.format(initialPosition.z),
        conditional(getProperty("useToolChanger"), hFormat.format(lengthOffset))
      );
    } else {
      writeBlock(
        gAbsIncModal.format(90),
        gMotionModal.format(0),
        conditional(getProperty("useToolChanger"), gFormat.format(43)), xOutput.format(initialPosition.x),
        yOutput.format(initialPosition.y),
        zOutput.format(initialPosition.z), conditional(getProperty("useToolChanger"), hFormat.format(lengthOffset))
      );
    }
