...
function onOpen() {
  ...
  if (getProperty("machineDefinition")) {
    ...
    var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-115.0001, 115.0001], preference: getProperty("preferTilt")});
...
function onSection() {
  ...
  if (getProperty("homeXYBetweenOperations")) { // force XY Z retracts
    if (!retracted) {
      writeRetract(Z);
    }
    writeRetract(X);
