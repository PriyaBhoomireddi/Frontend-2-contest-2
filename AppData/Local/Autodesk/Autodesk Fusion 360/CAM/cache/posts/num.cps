/**
  ...
  $Revision: 43215 c844add906baf64780f4c346e4d1e6e29f49db0a $
  $Date: 2021-03-04 04:46:18 $
...
function onSection() {
  ...
  if (insertToolCall ||
    ...
    if (tool.type == TOOL_TAP_LEFT_HAND) {
      writeBlock(
        gFormat.format(97), sOutput.format(spindleSpeed), mFormat.format(4)
      );
    } else {
      writeBlock(
  ...
  */
  if (!isFirstSection() && (getPreviousSection().workOffset != workOffset)) { // work offset changes)
    error(localize("Multiple work offsets are not supported."));
    return;
  }
