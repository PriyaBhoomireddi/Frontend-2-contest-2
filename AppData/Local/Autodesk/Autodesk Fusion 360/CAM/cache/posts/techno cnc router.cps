...
function writeComment(text) {
  if (getProperty("machineType") != "lc") {
    writeln(formatComment(text));
  }
}

function onOpen() {
  if (getProperty("machineType") == "hd") {
    unit = IN;
  }
  ...
  if (getProperty("machineType") == "hd" || getProperty("machineType") == "lc") {
    writeln("%");
  }
  ...
  if (getProperty("writeTools")) {
    ...
    if ((tools.getNumberOfTools() > 0) && (getProperty("machineType") != "hd")) {
  ...
  writeBlock(conditional(getProperty("machineType") == "hds", gFormat.format(600)));
}
...
function disableLengthCompensation(force) {
  if (lengthCompensationActive || force) {
    ...
    writeBlock(conditional(getProperty("machineType") == "hd2", gFormat.format(49)));
...
function onSection() {
  ...
  if ((workOffset > 0) && (getProperty("machineType") != "hd" && getProperty("machineType") != "lc")) {
    if (getProperty("machineType") == "hds") {
  ...
  if (hasParameter("operation-comment")) {
    ...
    if (comment && (getProperty("machineType") != "hd")) {
      writeComment(comment);
    }
  }
  ...
  if (insertToolCall) {
    ...
    if (getProperty("useToolChanger")) {
      ...
      if (!isFirstSection()) {
        ...
        writeComment("T" + toolFormat.format(tool.number), mFormat.format(6));
      }
    }
    if (tool.comment) {
  ...
  if (!isProbeOperation() &&
    ...
    writeBlock(conditional((getProperty("machineType") == "hds"), "(DLY,5)"));
  ...
  if (insertToolCall && (getProperty("machineType") == "hds")) {
    writeBlock(gFormat.format(27));
  }
  forceXYZ();
...
*/
  ...
  if ((getProperty("machineType") == "hd2") && (insertToolCall || !lengthCompensationActive || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis()))) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      if (getProperty("machineType") == "hd2") {
        writeBlock(gFormat.format(offsetCode), hFormat.format(lengthOffset));
      ...
      writeBlock(
        ...
        zOutput.format(initialPosition.z), conditional(getProperty("machineType") == "hd2", hFormat.format(lengthOffset))
      );
      lengthCompensationActive = true;
    }
...
function onLinear(_x, _y, _z, feed) {
  if ((getProperty("machineType") == "hd") && (pendingRadiusCompensation >= 0)) {
    error(localize("Radius compensation mode is not supported by HD series."));
    return;
  }
  var x = xOutput.format(_x);
  var y = yOutput.format(_y);
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  var i = iOutput.format((getProperty("machineType") != "hds") ? (cx - start.x) : (cx));
  var j = jOutput.format((getProperty("machineType") != "hds") ? (cy - start.y) : (cy));
  var k = kOutput.format((getProperty("machineType") != "hds") ? (cz - start.z) : (cz));
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(gAbsIncModal.format(90), conditional(getProperty("machineType") != "lc", gPlaneModal.format(17)), gMotionModal.format(clockwise ? 2 : 3), i, j, getFeed(feed));
      break;
    case PLANE_ZX:
      writeBlock(gAbsIncModal.format(90), conditional(getProperty("machineType") != "lc", gPlaneModal.format(18)), gMotionModal.format(clockwise ? 2 : 3), i, k, getFeed(feed));
      break;
    case PLANE_YZ:
      writeBlock(gAbsIncModal.format(90), conditional(getProperty("machineType") != "lc", gPlaneModal.format(19)), gMotionModal.format(clockwise ? 2 : 3), j, k, getFeed(feed));
      break;
    default:
      linearize(tolerance);
    }
    ...
    switch (getCircularPlane()) {
    case PLANE_XY:
      writeBlock(gAbsIncModal.format(90), conditional(getProperty("machineType") != "lc", gPlaneModal.format(17)), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), i, j, getFeed(feed));
      break;
    case PLANE_ZX:
      writeBlock(gAbsIncModal.format(90), conditional(getProperty("machineType") != "lc", gPlaneModal.format(18)), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), i, k, getFeed(feed));
      break;
    case PLANE_YZ:
      writeBlock(gAbsIncModal.format(90), conditional(getProperty("machineType") != "lc", gPlaneModal.format(19)), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), j, k, getFeed(feed));
      break;
    default:
      linearize(tolerance);
    }
    ...
    switch (getCircularPlane()) {
    case PLANE_XY:
      writeBlock(conditional(getProperty("machineType") != "lc", gPlaneModal.format(17)), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), "R" + rFormat.format(r), getFeed(feed));
      break;
    case PLANE_ZX:
      writeBlock(conditional(getProperty("machineType") != "lc", gPlaneModal.format(18)), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), "R" + rFormat.format(r), getFeed(feed));
      break;
    case PLANE_YZ:
      writeBlock(conditional(getProperty("machineType") != "lc", gPlaneModal.format(19)), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), "R" + rFormat.format(r), getFeed(feed));
      break;
    default:
      linearize(tolerance);
    }
...
function writeRetract() {
  if (getProperty("machineType") != "hds" && getProperty("machineType") != "lc") {
    return;
  }
  ...
  if (words) {
    gMotionModal.reset();
    writeBlock(gFormat.format(getProperty("machineType") == "lc" ? 92 : 79), words); // retract
  }
  zOutput.reset();
}
