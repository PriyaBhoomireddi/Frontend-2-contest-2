...
properties = {
  ...
  useCoolant: {
    title: "Use coolant",
    description: "Specifies to use coolant commands as specified in CAM.",
    type: "boolean",
    value: false,
    scope: "post"
  },
  ...
  disableArcs: {
    title: "Disable arcs",
    description: "Specifies to linearize arcs to G1 instead of using G2/G3.",
    type: "boolean",
    value: false,
    scope: "post"
  }
...
function onOpen() {
  ...
  if (!getProperty("safePositionMethod") == "G28") {
    if (is3D()) {
      ...
      for (var i = 0; i < numberOfSections; ++i) {
        var section = getSection(i);
        if (section.getGlobalZRange().getMaximum() > maximumClearanceHeight) {
          maximumClearanceHeight = section.getGlobalZRange().getMaximum();
          warning(localize("Retract height has been updated."));
        }
