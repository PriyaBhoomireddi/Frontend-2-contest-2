  ...
  */
  ...
  if (insertToolCall || retracted) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      writeBlock(gMotionModal.format(0), conditional(false, gFormat.format(43)), zOutput.format(initialPosition.z));
      writeBlock(
      ...
      writeBlock(
        gMotionModal.format(0),
        conditional(false, gFormat.format(43)),
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  // TAG: not supported for now - remove when supported
  linearize(tolerance);
  return;
  /*
...
function writeRetract() {
  ...
  if (method == "clearanceHeight") {
    if (!is3D()) {
      warning(localize("Make sure that the clearance height is set to a safe value for multi axis operations."));
  ...
  // special conditions
  if (retractAxes[0] || retractAxes[1]) { // Z use G28
    method = "G53";
  }
...
function onClose() {
  ...
  /* // dangerous
  zOutput.reset();
  writeBlock(gFormat.format(0), zOutput.format(toPreciseUnit(1, MM))); // retract
  zOutput.reset();
  writeBlock(gFormat.format(0), zOutput.format(toPreciseUnit(3, MM))); // retract
*/
  writeRetract(Z);
