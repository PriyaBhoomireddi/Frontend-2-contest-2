...
properties = {
  ...
  material: {
    ...
    description: "Set the material type to either 'Hard' or 'Graphite'.",
    type: "enum",
    values: [
      {title: "Hard", id: "hard"},
...
function onOpen() {
  ...
  if (programName) {
    writeln("(" + filterText(String(programName).toUpperCase(), permittedCommentChars) + ")");
    if (programComment) {
      writeln("(" + filterText(String(programComment).toUpperCase(), permittedCommentChars) + ")");
    }
    try {
      lastSubprogram = getAsInt(programName);
    } catch (e) {
      lastSubprogram = 0;
    }
  } else {
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || forceSmoothing || breakControl) {
    
    if ((insertToolCall && !isFirstSection()) || forceSmoothing || breakControl) {
      setSmoothing(false);
  ...
  if (!isProbeOperation() &&
    ...
    startSpindle(spindleSpeed, tool.clockwise);
  ...
  if (workOffset > 0) {
    if (workOffset > 6) {
      ...
      if (p > 300) {
        ...
        if (workOffset != currentWorkOffset) {
          wcsCode = gFormat.format(54.1) + " P" + p; // G54.1P
...
function startSpindle(_spindleSpeed, clockwise) {
  if ((Math.abs(_spindleSpeed - currentRPM) > minSpeedChange) && (_spindleSpeed != minSpeedChange)) {
    writeBlock(sOutput.format(minSpeedChange), mFormat.format(clockwise ? 3 : 4));
    onDwell(10);
  }
  writeBlock(sOutput.format(_spindleSpeed), mFormat.format(clockwise ? 3 : 4));
  onDwell(10);
  currentRPM = _spindleSpeed;
}

function onSpindleSpeed(_spindleSpeed) {
  startSpindle(_spindleSpeed, tool.clockwise);
}
...
function onCommand(command) {
  ...
  case COMMAND_STOP_SPINDLE:
    if (currentRPM > 5000) {
      startSpindle(5000, tool.clockwise);
    }
