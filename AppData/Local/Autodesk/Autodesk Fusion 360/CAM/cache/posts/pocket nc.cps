/**
  ...
  $Revision: 43260 ad91be746989d99b7c13d2b81b4c6cbf1c645c86 $
  $Date: 2021-03-29 08:46:55 $
...
function onOpen() {
  ...
  // optimizeMachineAngles2(getProperty("useTCPMode") ? 0 : 1); // TCP mode
...
function getWorkPlaneMachineABC(workPlane) {
  ...
  var tcp = false; // getProperty("useTCPMode");
...
function onSection() {
  ...
  if (!retracted && !insertToolCall) {
    if (getCurrentPosition().z < initialPosition.z) {
      writeBlock(gMotionModal.format(getProperty("useG0") ? 0 : 1), zOutput.format(initialPosition.z), conditional(!getProperty("useG0"), getFeed(highFeedrate)));
    }
  }
  ...
  if (insertToolCall || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) { // && !getProperty("useTCPMode")) {
      writeBlock(
        gAbsIncModal.format(90),
        gMotionModal.format(getProperty("useG0") ? 0 : 1), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y), conditional(!getProperty("useG0"), getFeed(highFeedrate))
      );
      writeBlock(gMotionModal.format(getProperty("useG0") ? 0 : 1), gFormat.format(43), zOutput.format(initialPosition.z), hFormat.format(lengthOffset));
    } else {
