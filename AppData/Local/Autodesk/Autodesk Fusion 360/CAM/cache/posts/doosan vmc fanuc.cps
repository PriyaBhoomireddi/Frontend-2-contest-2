...
description = "Doosan 3/5-axis VMC (FANUC)";
vendor = "Doosan";
vendorUrl = "http://www.doosanmachinetools.com";
legal = "Copyright (C) 2012-2021 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45702;

longDescription = "Post for Doosan 3/5-axis VMC post with Fanuc control.";
...
properties = {
  ...
  useNanoSmoothing: {
    title: "Use Nano smoothing",
    description: "Enables nano smoothing mode.",
    type: "boolean",
    value: false,
    scope: "post"
  },
...
var useRotaryScale = false; // output C-axis on a rotary scale
var useRotarySign = true; // C-axis uses sign of angle to determine direction
var probeCode = {code:mFormat, value:165};
...
function onOpen() {
  ...
  smoothing = getProperty("useSmoothing") ? 1 : 0;
  ...
  if (getProperty("has5Axis")) { // note: setup your machine here
    ...
    var cAxis;
    if (useRotaryScale) { // output C-axis on rotary scale
      cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:true, range:[0, 360]});
    } else { // output C-axis on linear scale
      cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1]});
    }
...
var currentRSmoothing = 0;
var rSmoothing = 0;

function setSmoothing(mode) {
  if ((mode && (currentSmoothing != 0)) || (!mode && (currentSmoothing == 0))) {
    return false;
  }
  ...
  if (smoothing == 2) {
    if (mode) {
      ...
      if (currentSection.isMultiAxis()) {
        writeBlock(
          ...
          "X0", "Y0", "Z0",
          conditional(machineConfiguration.isMachineCoordinate(0), "A0"),
          conditional(machineConfiguration.isMachineCoordinate(1), "B0"),
          conditional(machineConfiguration.isMachineCoordinate(2), "C0")
        );
      } else {
        writeBlock(
          gFormat.format(5.1), "Q3",
    ...
    if (mode) {
      currentSmoothing = smoothing;
      currentRSmoothing = rSmoothing;
      var rCode = "";
      if (rSmoothing > 0) {
        rCode = "R" + rSmoothing;
      } else if ((rSmoothing == 0) && hasParameter("operation:tolerance")) {
      ...
      writeBlock(gFormat.format(5.1), "Q1", rCode);
    } else {
      currentSmoothing = 0;
      writeBlock(gFormat.format(5.1), "Q0");
    }
        ...
        if (!useMultiAxisFeatures) {
          ...
          writeBlock(
            ...
            conditional(machineConfiguration.isMachineCoordinate(2),
              cOutput.format(getDirectionalABC(currentMachineABC.z, abc.z, cOutput)))
          );
...
function setWorkPlane(section, abc) {
  ...
  if (useMultiAxisFeatures) {
    ...
    if (machineConfiguration.isMultiAxisConfiguration() && (useABCPrepositioning || section.isMultiAxis() || abc.isZero())) {
      var angles = abc.isNonZero() ? (section.isMultiAxis() ? section.getInitialToolAxisABC() : getWorkPlaneMachineABC(section.workPlane, false, false)) : abc;
      gMotionModal.reset();
      writeBlock(
        gMotionModal.format(0),
        aOutput.format(angles.x),
        bOutput.format(angles.y),
        cOutput.format(getDirectionalABC(currentMachineABC.z, angles.z, cOutput))
      );
    ...
    if (abc.isNonZero()) {
      ...
      if (!isPrepositioned || !section.isMultiAxis()) {
      ...
      if (!isPrepositioned) {
        writeBlock(
          gMotionModal.format(0),
          conditional(machineConfiguration.isMachineCoordinate(0), aOutput.format(0)),
          conditional(machineConfiguration.isMachineCoordinate(1), bOutput.format(0)),
          cOutput.format(getDirectionalABC(currentMachineABC.z, angles.z, cOutput))
        );
      }
    }
...
function getPreferenceWeight(_abc) {
  ...
  var preference = new Array(axis[0].getPreference(), axis[1].getPreference(), axis[2].getPreference());
...
function onSection() {
  ...
  var forceSmoothing = (smoothing != currentSmoothing) || (rSmoothing != currentRSmoothing) || ((smoothing != 0) &&
  ...
  if (insertToolCall) {
    activeCoolants = []; // force coolant on tool change
    forceWorkPlane();
...
function protectedProbeMove(_cycle, x, y, z) {
  ...
  if (_z && z >= getCurrentPosition().z) {
    writeBlock(probeCode.code.format(probeCode.value), "P" + 9810, _z, getFeed(cycle.feedrate)); // protected positioning move
  }
  if (_x || _y) {
    writeBlock(probeCode.code.format(probeCode.value), "P" + 9810, _x, _y, getFeed(highFeedrate)); // protected positioning move
  }
  if (_z && z < getCurrentPosition().z) {
    writeBlock(probeCode.code.format(probeCode.value), "P" + 9810, _z, getFeed(cycle.feedrate)); // protected positioning move
  }
...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint() || isProbeOperation()) {
    ...
    case "probing-x":
      ...
      writeBlock(
        probeCode.code.format(probeCode.value), "P" + 9811,
    ...
    case "probing-x-wall":
      protectedProbeMove(cycle, x, y, z);
      writeBlock(
        probeCode.code.format(probeCode.value), "P" + 9812,
    ...
    case "probing-xy-circular-boss":
      protectedProbeMove(cycle, x, y, z);
      writeBlock(
        probeCode.code.format(probeCode.value), "P" + 9814,
    ...
    case "probing-xy-circular-partial-boss":
      protectedProbeMove(cycle, x, y, z);
      writeBlock(
        probeCode.code.format(probeCode.value), "P" + 9823,
    ...
    case "probing-xy-inner-corner":
      ...
      protectedProbeMove(cycle, x, y, z - cycle.depth);
      writeBlock(
        probeCode.code.format(probeCode.value), "P" + 9815, xOutput.format(cornerX), yOutput.format(cornerY),
    ...
    case "probing-xy-outer-corner":
      ...
      protectedProbeMove(cycle, x, y, z - cycle.depth);
      writeBlock(
        probeCode.code.format(probeCode.value), "P" + 9816, xOutput.format(cornerX), yOutput.format(cornerY),
        conditional(cornerI != 0, "I" + xyzFormat.format(cornerI)),
        conditional(cornerJ != 0, "J" + xyzFormat.format(cornerJ)),
        "Q" + xyzFormat.format(cycle.probeOvertravel),
        getProbingArguments(cycle, true)
    ...
    case "probing-x-plane-angle":
      protectedProbeMove(cycle, x, y, z - cycle.depth);
      writeBlock(
        probeCode.code.format(probeCode.value), "P" + 9843,
        "X" + xyzFormat.format(x + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2)),
    ...
    case "probing-xy-pcd-hole":
      protectedProbeMove(cycle, x, y, z);
      writeBlock(
        probeCode.code.format(probeCode.value), "P" + 9819,
...
function onCycleEnd() {
  if (isProbeOperation()) {
    zOutput.reset();
    gMotionModal.reset();
    writeBlock(probeCode.code.format(probeCode.value), "P" + 9810, zOutput.format(cycle.retract)); // protected retract move
  } else {
    if (cycleSubprogramIsActive) {
      subprogramEnd();
      cycleSubprogramIsActive = false;
...
function getDirectionalABC(_startAngle, _endAngle, _output) {
  ...
  // linear scale or unsigned angles are output
  if (!useRotaryScale || !useRotarySign) {
    return signedAngle;
  }
...
function onRapid5D(_x, _y, _z, _a, _b, _c) {
  ...
  var c = cOutput.format(getDirectionalABC(currentMachineABC.z, _c, cOutput));
  if (x || y || z || a || b || c) {
...
function onCommand(command) {
    ...
    if (!axisIsClamped) {
      if (machineConfiguration.isMultiAxisConfiguration() && (machineConfiguration.getNumberOfAxes() >= 4)) {
        ...
        if (machineConfiguration.getNumberOfAxes() == 5) {
          writeBlock(mFormat.format(38)); // lock 5th-axis motion
        }
      }
      axisIsClamped = true;
    }
  ...
  case COMMAND_UNLOCK_MULTI_AXIS:
    if (axisIsClamped) {
      if (machineConfiguration.isMultiAxisConfiguration() && (machineConfiguration.getNumberOfAxes() >= 4)) {
        ...
        if (machineConfiguration.getNumberOfAxes() == 5) {
          writeBlock(mFormat.format(39)); // unlock 5th-axis motion
        }
      }
      axisIsClamped = false;
    }
  ...
  case COMMAND_BREAK_CONTROL:
    if (!toolChecked) { // avoid duplicate COMMAND_BREAK_CONTROL
      ...
      setWorkPlane(currentSection, new Vector(0, 0, 0));
    
      writeBlock(
        probeCode.code.format(probeCode.value),
        "P" + 9028,
...
function writeRetract() {
  ...
  */
  ...
  if (words.length > 0) {
    ...
    case "G30":
      gAbsIncModal.reset();
      writeBlock(gMotionModal.format(0), gFormat.format(91), gAbsIncModal.format(30), words);
      writeBlock(gAbsIncModal.format(90));
      break;
    default:
...
function onClose() {
  ...
  setWorkPlane(currentSection, new Vector(0, 0, 0)); // reset working plane
  ...
  writeln("");
  writeln("N" + Math.max(sequenceNumber, 9999));
  writeln("");
