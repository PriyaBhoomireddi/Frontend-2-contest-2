...
function onOpen() {
  ...
  machineConfiguration.setModel("TMC-AX");
...
function onSection() {
  ...
  if (tempSpindle == SPINDLE_LIVE) {
    ...
    if (insertToolCall || wcsOut || feedMode) {
      ...
      writeBlock(gMotionModal.format(0), gFormat.format(28), "C" + abcFormat.format(0)); // unwind c-axis
    ...
    if (insertToolCall) {
      if ((tool.maximumSpindleSpeed > 0) && (currentSection.getTool().getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED)) {
        ...
        writeBlock(gFormat.format(50),
          ...
          spOutput.format(getCode("SELECT_SPINDLE", getSpindle(true)))
        );
        sOutput.reset();
      }
    }
  ...
  } else if (machineState.useXZCMode && yAxisWasEnabled) {
...
function startSpindle(tappingMode, forceRPMMode, initialPosition, rpm) {
  ...
  writeBlock(
    ...
    spindleDir,
    spOutput.format(getCode("SELECT_SPINDLE", getSpindle(false)))
  );
}
...
function onClose() {
  ...
  if (liveTool) {
    if (!stockTransferIsActive) {
      writeBlock(gFormat.format(28), "C" + abcFormat.format(0)); // unwind
      cAxisEngageModal.reset();
      writeBlock(cAxisEngageModal.format(getCode("DISABLE_C_AXIS", getSpindle(true))));
    }
  }
