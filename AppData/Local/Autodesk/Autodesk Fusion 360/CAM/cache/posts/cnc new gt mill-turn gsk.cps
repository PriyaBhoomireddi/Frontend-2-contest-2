...
function onOpen() {
  ...
  if (true) {
    ...
    var cAxisMain = createAxis({coordinate:2, table:true, axis:[0, 0, (getProperty("reverseCAxis") ? -1 : 1)], cyclic:true, preference:0}); // C axis is modal between primary and secondary spindle

    var bAxisSub = createAxis({coordinate:1, table:false, axis:[0, -1, 0], range:[-0.001, 180.001], preference:0});
    var cAxisSub = createAxis({coordinate:2, table:true, axis:[0, 0, (getProperty("reverseCAxis") ? -1 : 1)], cyclic:true, preference:0}); // C axis is modal between primary and secondary spindle
...
function invertAxes(activate, polarMode) {
  if (activate) {
    ...
    xFormat = createFormat({decimals:(unit == MM ? 3 : 4), forceDecimal:true, scale:2 * xDirection});
...
function onSection() {
  ...
  if (!(stockTransferIsActive && partCutoff && gotSecondarySpindle)) {
  ...
  if (insertToolCall) {
    ...
    if (gotMultiTurret) {
      ...
      case 104:
        ...
        xFormat = createFormat({decimals:(unit == MM ? 3 : 4), forceDecimal:true, scale:2}); // inverted diameter mode
        xOutput = createVariable({prefix:"X"}, xFormat);
...
function updateMachiningMode(section) {
  ...
  if ((section.getType() == TYPE_MILLING) && !section.isMultiAxis()) {
    ...
    } else if (getMachiningDirection(section) == MACHINING_DIRECTION_RADIAL) { // G18 plane
      if (!gotYAxis) {
...
function onRapid(_x, _y, _z) {
  ...
  if (x || y || z) {
    ...
    if (pendingRadiusCompensation >= 0) {
      ...
      var ccLeft = (((getSpindle(true) == SPINDLE_SUB) && reverseAxes) || (xDirection == -1)) ? 42 : 41;
      var ccRight = (((getSpindle(true) == SPINDLE_SUB) && reverseAxes) || (xDirection == -1)) ? 41 : 42;
      if (useG1) {
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  var directionCode;
  if (((getSpindle(true) == SPINDLE_SUB) && reverseAxes) || xDirection == -1) {
...
function cycleBarPull() {
  ...
  case "secondary-spindle-return":
    ...
    onDwell(cycle.dwell);
    writeBlock(gMotionModal.format(1), zOutput.format(cycle.feedPosition), getFeed(cycle.feedrate), formatComment("BAR PULL"));
    writeBlock(mFormat.format(getCode("CLAMP_CHUCK", getSpindle(true))), formatComment("CLAMP MAIN CHUCK"));
    onDwell(cycle.dwell);
    if (cycle.feedPosition < 0) {
...
function getG17Code() {
  return machineState.usePolarMode ? 18 : 18;
}
...
function writeRetract() {
  ...
  var _xHome = 0;
  var _yHome = 0;
  var _zHome = getProperty("useG28Zhome") ? 0 : getProperty("zHomePosition");
...
function onClose() {
  ...
  if (stockTransferIsActive && gotSecondarySpindle) {
    writeBlock(
