...
function onOpen() {
  ...
  if (programName) {
    var programONumber;
    try {
      programONumber = getAsInt(programName);
    } catch (e) {
      error(localize("Program name must be a number."));
      return;
    }
    if (!((programONumber >= 1) && (programONumber <= 9999))) {
      error(localize("Program number is out of range."));
      return;
    }
    if (programComment) {
      writeln("O" + oFormat.format(programONumber) + " (" + filterText(String(programComment).toUpperCase(), permittedCommentChars) + ")");
    } else {
      writeln("O" + oFormat.format(programONumber));
    }
  } else {
...
function onSection() {
  ...
  if (insertToolCall || forceSmoothing) {
...
*/
  ...
  if (insertToolCall || !lengthCompensationActive || retracted) {
  ...
  if (isFirstCyclePoint() || isProbeOperation()) {
    ...
    case "tapping":
      ...
      if (getProperty("usePitchForTapping")) {
        writeBlock(
          gRetractModal.format(98), gFeedModeModal.format(95), gAbsIncModal.format(90), gCycleModal.format((tool.type == TOOL_TAP_LEFT_HAND) ? 74 : 84),
          getCommonCycle(x, y, z, cycle.retract),
          "P" + milliFormat.format(P),
    ...
    case "left-tapping":
      ...
      if (getProperty("usePitchForTapping")) {
        writeBlock(
          gRetractModal.format(98), gFeedModeModal.format(95), gAbsIncModal.format(90), gCycleModal.format(74),
          getCommonCycle(x, y, z, cycle.retract),
          "P" + milliFormat.format(P),
          pitchOutput.format(tool.threadPitch)
        );
    ...
    case "right-tapping":
      ...
      if (getProperty("usePitchForTapping")) {
        writeBlock(
          gRetractModal.format(98), gFeedModeModal.format(95), gAbsIncModal.format(90), gCycleModal.format(84),
          getCommonCycle(x, y, z, cycle.retract),
          "P" + milliFormat.format(P),
          pitchOutput.format(tool.threadPitch)
        );
    ...
    if (text) {
      if (blockCount < MINIMUM_SUBPROGRAM_SIZE) { // too small
        ...
        if ((id == undefined) || partial) {
          if (partial) {
            redirectToBuffer();
            writeBlock("P" + (programId + id + 1), mFormat.format(98));
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      ...
      writeBlock(gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, iOutput.format(cx - start.x, 0), jOutput.format(cy - start.y, 0), feedOutput.format(feed));
      break;
    case PLANE_ZX:
      var _z = zOutput.format(z);
      if (_z) {
      ...
      writeBlock(gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, iOutput.format(cx - start.x, 0), kOutput.format(cz - start.z, 0), feedOutput.format(feed));
      break;
    case PLANE_YZ:
      var _z = zOutput.format(z);
      if (_z) {
      ...
      writeBlock(gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, jOutput.format(cy - start.y, 0), kOutput.format(cz - start.z, 0), feedOutput.format(feed));
      break;
    default:
      linearize(tolerance);
    }
    ...
    case PLANE_XY:
      ...
      writeBlock(gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, "R" + rFormat.format(r), feedOutput.format(feed));
      break;
    case PLANE_ZX:
      var _z = zOutput.format(z);
      if (_z) {
      ...
      writeBlock(gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, "R" + rFormat.format(r), feedOutput.format(feed));
      break;
    case PLANE_YZ:
      var _z = zOutput.format(z);
      if (_z) {
      ...
      writeBlock(gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), _z, "R" + rFormat.format(r), feedOutput.format(feed));
      break;
    default:
      linearize(tolerance);
    }
...
function onClose() {
  ...
  for (var i = 0; i < subprograms.length; ++i) {
    ...
    writeln("");
    writeln("O" + oFormat.format(programId + i + 1));
