...
function onSection() {
  ...
  if (tool.type != TOOL_PROBE && (insertToolCall ||
    isFirstSection() || isSpindleSpeedDifferent())) {
    if (spindleSpeed < 0) {
  ...
  if (insertToolCall) {
    ...
    writeBlock(wcsCode, gAbsIncModal.format(90), gPlaneModal.format(17), gFormat.format(80), gFeedModeModal.format(currentFeedMode),
    ...
    if (tool.type != TOOL_PROBE && isSpindleSpeedDifferent()) {
      startSpindle();
    }
  }
...
function onCyclePoint(x, y, z) {
  ...
  case "thread-turning":
    ...
    var cuttingAngle = 0; // Angle is not stored with tool. toDeg(tool.getTaperAngle());
...
function writeRetract() {
  ...
  var _useG28 = false;
  if (getProperty("useG28") == "true") {
    _useG28 = true;
  } else if (getProperty("useG28") == "clearanceHeight") {
    if (!is3D()) {
