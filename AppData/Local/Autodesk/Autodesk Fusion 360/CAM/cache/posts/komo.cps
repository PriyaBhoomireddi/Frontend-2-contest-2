...
var gTappingModal = createModal({}, gFormat); // G63-G64
...
function setSmoothing(mode) {
  ...
  writeBlock(gFormat.format(8), mode ? "P1" : "P0");
  return true;
}
...
function subprogramDefine(_initialPosition, _abc, _retracted, _zIsOutput) {
  ...
  if (currentSection.isPatterned && currentSection.isPatterned() && (getProperty("useSubroutines") == "patterns")) {
    ...
    if (usePattern) {
      ...
      // call subprogram
      writeBlock(gFormat.format(65), "P" + oFormat.format(currentSubprogram));
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    */
    ...
    if (tool.comment) {
      ...
      writeBlock("T" + toolFormat.format(102));
    }
    ...
    if (false /*getProperty("preloadTool")*/) {
  ...
  if (tool.type != TOOL_PROBE &&
    ...
    if (true /*!tapping*/) {
      spindleCode = formatWords(sOutput.format(spindleSpeed), mFormat.format(tool.clockwise ? 3 : 4));
    }
...
function onCycleEnd() {
  if (isProbeOperation()) {
    ...
    if (!cycleExpanded) {
      writeBlock(
        conditional(!getProperty("useG95"), gFeedModeModal.format(94)),
        conditional(tapping, gTappingModal.format(64)),
