...
properties = {
  ...
  sequenceNumberOnlyOnToolChange: {
    title: "Sequence number at tool change only",
    description: "Use sequence numbers at a tool change only.",
    type: "boolean",
    value: false,
    scope: "post"
  },
...
function writeBlock() {
  ...
  if (getProperty("showSequenceNumbers") && !getProperty("sequenceNumberOnlyOnToolChange")) {
...
function onOpen() {
  ...
  if (getProperty("rotaryTable") != "none") { // note: setup your machine here
    if (getProperty("rotaryTable") == "4axis") {
      ...
      var aAxis = createAxis({coordinate:0, table:true, axis:[0, 0, 1], cyclic:true, preference:0});
      var bAxis = createAxis({coordinate:1, table:true, axis:[0, 1, 0], cyclic:false, range:[-30, 110], preference:0});
...
function onRapid5D(_x, _y, _z, _a, _b, _c) {
  if (!currentSection.isOptimizedForMachine() || (getProperty("rotaryTable") == "5axis")) {
...
var inverseTimeFormat = createFormat({decimals:(unit == MM ? 0 : 1), forceDecimal:true});
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    ...
    if (getProperty("rotaryTable") == "5axis") {
...
function writeRetract() {
  ...
  // special conditions
  if (method == "G28" && !retractAxes[0] && retractAxes[1] && !retractAxes[2]) { // Y-axis by itself uses G30
    method = "G30";
  } else {
