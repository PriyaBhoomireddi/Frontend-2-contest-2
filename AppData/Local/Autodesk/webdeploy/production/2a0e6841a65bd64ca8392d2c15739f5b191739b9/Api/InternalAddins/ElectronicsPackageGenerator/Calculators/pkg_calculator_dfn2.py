...
class PackageCalculatorDfn2(pkg_calculator.PackageCalculator):
	
	# initialize the data members
	def __init__(self, pkg_type: str):
		super().__init__(pkg_type)
		self.pkg_type = constant.PKG_TYPE_DFN2
	...
	def get_footprint(self):
		...
		if self.ui_data['componentFamily'] == constant.COMP_FAMILY_DIODE:
			if pin_pitch + pad_width > body_width:
				line_x = pin_pitch/2 + pad_width/2
			else :
				line_x = body_width/2
			line_start_x = - (line_x + ipc_rules.SILKSCREEN_ATTRIBUTES['Clearance'])
	...
	def get_ipc_package_name(self):
		...
		if (self.ui_data['componentFamily'] == constant.COMP_FAMILY_INDUCTOR):
			family_name = 'INDDFN' 
		if (self.ui_data['componentFamily'] == constant.COMP_FAMILY_CRYSTAL):
			family_name = 'XTALDFN' 
...
pkg_calculator.calc_factory.register_calculator(constant.PKG_TYPE_DFN2, PackageCalculatorDfn2) 