...
class PackageCalculatorSon(pkg_calculator.PackageCalculator):
	...
	def get_footprint(self):
		...
		if self.ui_data['hasCustomFootprint'] :
			...
			pad_width, pad_height, pin_pitch = self.get_footprint_smd_data(self.ui_data['bodyWidthMin'], self.ui_data['bodyWidthMax'],
	...
	def get_ipc_package_description(self):
		...
		short_description = str(self.ui_data['horizontalPadCount']) + '-' + pkg_name+ ' (DFN), '
		...
		full_description = str(self.ui_data['horizontalPadCount']) + '-pin '+ pkg_name + ' (DFN) package with '
...
pkg_calculator.calc_factory.register_calculator(constant.PKG_TYPE_SON, PackageCalculatorSon) 