...
class PackageCommandSoj(PackageCommand.PackageCommand):
    def __init__(self, name: str, options: dict):
        super().__init__(name, options)

        #overwrite some specific settings of this command. 
        self.cmd_id = 'cmd_id_Soj'
        self.cmd_name = _LCLZ('CmdNameSoj', 'SOJ Generator')
        self.cmd_description = _LCLZ('CmdDescSoj', 'Generate SOJ Packages')
    ...
    def get_defalt_ui_data(self):
        ...
        ui_data['type'] = constant.PKG_TYPE_SOJ
        ui_data['horizontalPadCount'] = 20
        ui_data['horizontalLeadToLeadSpanMin'] = 0.851
        ui_data['horizontalLeadToLeadSpanMax'] = 0.881
        ui_data['terminalCenterToCenterDistance'] = 0.68
        ui_data['roundedPadCornerSize'] = 40
        ui_data['padHeightMin'] = 0.036
        ui_data['padHeightMax'] = 0.051
        ui_data['verticalPinPitch'] = 0.127
        ui_data['bodyWidthMin'] = 0.742
        ui_data['bodyWidthMax'] = 0.762
        ui_data['bodyLengthMin'] = 1.26
        ui_data['bodyLengthMax'] = 1.3
        ui_data['bodyHeightMin'] = 0
        ui_data['bodyHeightMax'] = 0.375
        ui_data['bodyOffsetMin'] = 0.101
        ui_data['bodyOffsetMax'] = 0.101
        ui_data['customPadLength'] = 0.243
        ui_data['customPadWidth'] = 0.061
        ui_data['customPadSpan1'] = 0.953
    ...
    def on_create(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs):
        ...
        package_tab_cmd_inputs = inputs.addTabCommandInput('sojPackageTab', _LCLZ('package', 'Package'))
        soj_inputs = package_tab_cmd_inputs.children

        # Create image input.
        labeled_image = soj_inputs.addImageCommandInput('sojImage', '', "Resources/img/SOJ-Labeled.png")
        labeled_image.isFullWidth = True
        labeled_image.isVisible = True

        soj_inputs.addIntegerSpinnerCommandInput('horizontalPadCount', _LCLZ('#Pins', '# Pins'), 2, 50 , 2, int(self.ui_data['horizontalPadCount']))

        # Create dropdown input with test list style.
        pad_shape = soj_inputs.addDropDownCommandInput('padShape', _LCLZ('padShape', 'Pad Shape'), adsk.core.DropDownStyles.TextListDropDownStyle)
        ...
        rounded_corner = soj_inputs.addValueInput('roundedPadCornerSize', _LCLZ('padRoundness', 'Pad Roundness (%)'), '', adsk.core.ValueInput.createByReal(self.ui_data['roundedPadCornerSize']))
        rounded_corner.isVisible = True if constant.SMD_PAD_SHAPE.get('RoundedRectangle') == self.ui_data['padShape'] else False

        pin_pitch = soj_inputs.addValueInput('verticalPinPitch', 'e', ao.units_manager.defaultLengthUnits, adsk.core.ValueInput.createByReal(self.ui_data['verticalPinPitch']))
        pin_pitch.tooltip = _LCLZ('pinPitch', 'Pin Pitch')
        terminal_centre = soj_inputs.addValueInput('terminalCenterToCenterDistance', 'E2', ao.units_manager.defaultLengthUnits, adsk.core.ValueInput.createByReal(self.ui_data['terminalCenterToCenterDistance']))
        terminal_centre.tooltip = _LCLZ('terminalCCGap', 'Terminal Center-Center Gap')

        # table
        table = soj_inputs.addTableCommandInput('bodyDimensionTable', 'Table', 3, '1:2:2')
        ...
        # Create dropdown input with test list style.
        map_silkscreen = soj_inputs.addDropDownCommandInput('silkscreenMappingTypeToBody', _LCLZ('mapSilkscreen', 'Map Silkscreen to Body'), adsk.core.DropDownStyles.TextListDropDownStyle)
        ...
        err_message = soj_inputs.addTextBoxCommandInput('errMessage', '', '', 2, True)
        err_message.isFullWidth = True

        if not self.only_3d_model_generator:
            # Create a custom footprint tab input.
            footprint_tab_cmd_inputs = inputs.addTabCommandInput('sojFootprintTab', _LCLZ('footprint', 'Footprint'))
            ...
            # Create image input.
            custom_footprint_image = custom_footprint_inputs.addImageCommandInput('customSojImage', '', "Resources/img/SOIC-Custom-Footprint.png")
...
PackageCommand.cmd_factory.register_command(constant.PKG_TYPE_SOJ, PackageCommandSoj)
