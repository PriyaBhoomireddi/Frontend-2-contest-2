...
class PackageCommandSon(PackageCommand.PackageCommand):
    def __init__(self, name: str, options: dict):
        super().__init__(name, options)

        #overwrite some specific settings of this command. 
        self.cmd_id = 'cmd_id_Son'
        self.cmd_name = _LCLZ('CmdNameSon', 'SON Generator')
        self.cmd_description = _LCLZ('CmdDescSon', 'Generate SON Packages')
    ...
    def get_defalt_ui_data(self):
        ...
        ui_data['type'] = constant.PKG_TYPE_SON
        ui_data['horizontalPadCount'] = 14
        ...
        ui_data['bodyWidthMin'] = 0.29
        ui_data['bodyWidthMax'] = 0.31
        ...
        ui_data['thermalPadLength'] = 0.15
        ui_data['thermalPadWidth'] = 0.34
        ui_data['thermalPadSolderPasteOverride'] = False
        ui_data['thermalPadSolderPasteStencilApertureGapX'] = 0
        ui_data['thermalPadSolderPasteStencilApertureGapY'] = 0.026
        ui_data['thermalPadSolderPasteStencilApertureLength'] = 0.132
        ui_data['thermalPadSolderPasteStencilApertureWidth'] = 0.1160
    ...
    def on_create(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs):
        ...
        package_tab_cmd_inputs = inputs.addTabCommandInput('sonPackageTab', _LCLZ('package', 'Package'))
        son_inputs = package_tab_cmd_inputs.children

        # Create image input.
        labeled_image = son_inputs.addImageCommandInput('sonImage', '', "Resources/img/SON-Labeled.png")
        labeled_image.isFullWidth = True
        labeled_image.isVisible = True

        son_inputs.addIntegerSpinnerCommandInput('horizontalPadCount', _LCLZ('#Pins', '# Pins'), 2, 50 , 2, int(self.ui_data['horizontalPadCount']))

        # Create dropdown input with test list style.
        pad_shape = son_inputs.addDropDownCommandInput('padShape', _LCLZ('padShape', 'Pad Shape'), adsk.core.DropDownStyles.TextListDropDownStyle)
        ...
        pin_pitch = son_inputs.addValueInput('verticalPinPitch', 'e', ao.units_manager.defaultLengthUnits, adsk.core.ValueInput.createByReal(self.ui_data['verticalPinPitch']))
        pin_pitch.tooltip = _LCLZ('pinPitch', 'Pin Pitch')

        # table
        table = son_inputs.addTableCommandInput('bodyDimensionTable', 'Table', 3, '1:2:2')
        ...
        # Create dropdown input with test list style.
        map_silkscreen = son_inputs.addDropDownCommandInput('silkscreenMappingTypeToBody', _LCLZ('mapSilkscreen', 'Map Silkscreen to Body'), adsk.core.DropDownStyles.TextListDropDownStyle)
        ...
        err_message = son_inputs.addTextBoxCommandInput('errMessage', '', '', 2, True)
        err_message.isFullWidth = True

        if not self.only_3d_model_generator:
            # Create a custom footprint tab input.
            footprint_tab_cmd_inputs = inputs.addTabCommandInput('sonFootprintTab', _LCLZ('footprint', 'Footprint'))
            ...
            # Create image input.
            custom_footprint_image = custom_footprint_inputs.addImageCommandInput('customSonImage', '', "Resources/img/Son-Custom-Footprint.png") #TODO:update image
        ...
        thermal_tab_cmd_inputs = inputs.addTabCommandInput('sonThermalTab', _LCLZ('thermal', 'Thermal'))
        thermal_pad_inputs = thermal_tab_cmd_inputs.children
        # Create image input.
        labeled_image = thermal_pad_inputs.addImageCommandInput('sonImage', '', "Resources/img/SON-Labeled.png")
        labeled_image.isFullWidth = True
        labeled_image.isVisible = True       
...
PackageCommand.cmd_factory.register_command(constant.PKG_TYPE_SON, PackageCommandSon)
