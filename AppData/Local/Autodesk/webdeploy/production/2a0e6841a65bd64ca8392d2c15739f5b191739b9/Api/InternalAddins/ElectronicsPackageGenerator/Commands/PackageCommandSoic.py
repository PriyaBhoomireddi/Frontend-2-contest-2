...
class PackageCommandSoic(PackageCommand.PackageCommand):
    def __init__(self, name: str, options: dict):
        super().__init__(name, options)

        #overwrite some specific settings of this command. 
        self.cmd_id = 'cmd_id_Soic'
        self.cmd_name = _LCLZ('CmdNameSoic', 'SOIC Generator')
        self.cmd_description = _LCLZ('CmdDescSoic', 'Generate SOIC Packages')
    ...
    def get_defalt_ui_data(self):
        ...
        ui_data['type'] = constant.PKG_TYPE_SOIC
        ui_data['horizontalPadCount'] = 20
        ui_data['horizontalLeadToLeadSpanMin'] = 1
        ui_data['horizontalLeadToLeadSpanMax'] = 1.065
        ui_data['roundedPadCornerSize'] = 40
        ui_data['padWidthMin'] = 0.04
        ui_data['padWidthMax'] = 0.127
        ui_data['padHeightMin'] = 0.033
        ui_data['padHeightMax'] = 0.051
        ui_data['verticalPinPitch'] = 0.127
        ui_data['bodyWidthMin'] = 0.74
        ui_data['bodyWidthMax'] = 0.76
        ui_data['bodyLengthMin'] = 1.26
        ui_data['bodyLengthMax'] = 1.3
        ui_data['bodyHeightMin'] = 0
        ui_data['bodyHeightMax'] = 0.265
        ui_data['bodyOffsetMin'] = 0.025
        ui_data['bodyOffsetMax'] = 0.025
        ui_data['hasThermalPad'] = False
        ui_data['thermalPadLength'] = 0.48
        ui_data['thermalPadWidth'] = 0.861
        ui_data['thermalPadSolderPasteOverride'] = False
        ui_data['thermalPadSolderPasteStencilApertureGapX'] = 0
        ui_data['thermalPadSolderPasteStencilApertureGapY'] = 0.065
        ui_data['thermalPadSolderPasteStencilApertureLength'] = 0.372
        ui_data['thermalPadSolderPasteStencilApertureWidth'] = 0.333
        ui_data['thermalPadSolderPasteStencilRowCount'] = 2
        ui_data['thermalPadSolderPasteStencilColCount'] = 1
        ui_data['thermalPadSolderPasteAreaCoverage'] = 60
        ui_data['customPadLength'] = 0.205
        ui_data['customPadWidth'] = 0.06
        ui_data['customPadSpan1'] = 1.136
    ...
    def on_create(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs):
        ...
        package_tab_cmd_inputs = inputs.addTabCommandInput('soicPackageTab', _LCLZ('package', 'Package'))
        soic_inputs = package_tab_cmd_inputs.children

        # Create image input.
        labeled_image = soic_inputs.addImageCommandInput('soicImage', '', "Resources/img/SOIC-SOP-Labeled.png")
        labeled_image.isFullWidth = True
        labeled_image.isVisible = True

        soic_inputs.addIntegerSpinnerCommandInput('horizontalPadCount', _LCLZ('#Pins', '# Pins'), 2, 50 , 2, int(self.ui_data['horizontalPadCount']))

        # Create dropdown input with test list style.
        pad_shape = soic_inputs.addDropDownCommandInput('padShape', _LCLZ('padShape', 'Pad Shape'), adsk.core.DropDownStyles.TextListDropDownStyle)
        ...
        rounded_corner = soic_inputs.addValueInput('roundedPadCornerSize', _LCLZ('padRoundness', 'Pad Roundness (%)'), '', adsk.core.ValueInput.createByReal(self.ui_data['roundedPadCornerSize']))
        rounded_corner.isVisible = True if constant.SMD_PAD_SHAPE.get('RoundedRectangle') == self.ui_data['padShape'] else False

        pin_pitch = soic_inputs.addValueInput('verticalPinPitch', 'e', ao.units_manager.defaultLengthUnits, adsk.core.ValueInput.createByReal(self.ui_data['verticalPinPitch']))
        pin_pitch.tooltip = _LCLZ('pinPitch', 'Pin Pitch')

        # table
        table = soic_inputs.addTableCommandInput('bodyDimensionTable', 'Table', 3, '1:2:2')
        ...
        # Create dropdown input with test list style.
        map_silkscreen = soic_inputs.addDropDownCommandInput('silkscreenMappingTypeToBody', _LCLZ('mapSilkscreen', 'Map Silkscreen to Body'), adsk.core.DropDownStyles.TextListDropDownStyle)
        ...
        err_message = soic_inputs.addTextBoxCommandInput('errMessage', '', '', 2, True)
        err_message.isFullWidth = True

        if not self.only_3d_model_generator:
            # Create a custom footprint tab input.
            footprint_tab_cmd_inputs = inputs.addTabCommandInput('soicFootprintTab', _LCLZ('footprint', 'Footprint'))
            ...
            # Create image input.
            custom_footprint_image = custom_footprint_inputs.addImageCommandInput('customSoicImage', '', "Resources/img/SOIC-Custom-Footprint.png")
        ...
        thermal_tab_cmd_inputs = inputs.addTabCommandInput('soicThermalTab', _LCLZ('thermal', 'Thermal'))
        thermal_pad_inputs = thermal_tab_cmd_inputs.children
        # Create image input.
        labeled_image = thermal_pad_inputs.addImageCommandInput('soicImage', '', "Resources/img/SOIC-SOP-Labeled.png")
        labeled_image.isFullWidth = True
        labeled_image.isVisible = True       
...
PackageCommand.cmd_factory.register_command(constant.PKG_TYPE_SOIC, PackageCommandSoic)
