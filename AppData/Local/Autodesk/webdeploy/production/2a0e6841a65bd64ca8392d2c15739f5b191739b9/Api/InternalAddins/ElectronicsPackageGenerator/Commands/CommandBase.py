...
class CommandBase:
    ...
    def __init__(self, name: str, options: dict):
        ...
        self.fusion_app: addin_utility.FusionApp = options.get('fusion_app', None)
        ...
        self.command_promoted_by_Default = options.get('command_promoted_by_Default', True)
        ...
        resource_path = os.path.join(addin_utility.ADDIN_ROOT_PATH, self.path, 'Resources', resources_folder)
        drop_resources_path = os.path.join(addin_utility.ADDIN_ROOT_PATH, self.path, 'Resources', drop_down_folder)
    ...
    def on_validate_inputs(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs,args: adsk.core.ValidateInputsEventArgs):
        ...
        When the user change the command input values. this function will be executed to do the validation.
        By referencing the inputs object you can easily access the ui component to do the check

        Args:
            command: reference to the command object
            inputs: quick reference directly to the commandInputs object
            args: need return the isValid value after the check.
        """
        pass
...
class _CommandValidateInputsHandler(adsk.core.ValidateInputsEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            command_ = event_args.firingEvent.sender
            command_inputs = command_.commandInputs
            self.cmd_object_.on_validate_inputs(command_, command_inputs,event_args)

        except:
            app = adsk.core.Application.get()
            ui = app.userInterface
            ui.messageBox('valdiate input failed: {}'.format(traceback.format_exc()))
...
class _CommandCreatedEventHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            on_validate_inputs_handler = _CommandValidateInputsHandler(self.cmd_object_)
            command_.validateInputs.add(on_validate_inputs_handler)
            self.cmd_object_.handlers.append(on_validate_inputs_handler)
