...
class PackageCommandHeaderStraight(PackageCommandHeader.PackageCommandHeader):
    def __init__(self, name: str, options: dict):
        super().__init__(name, options)

        #overwrite some specific settings of this command. 
        self.cmd_id = 'cmd_id_header_straight'
        self.cmd_name = _LCLZ('CmdNameHeaderStraight', 'Pin Header(Male) Straight Generator')
        self.cmd_description = _LCLZ('CmdDescHeaderStraight', 'Generate Pin Header(Male) Straight Packages')
    ...
    def get_defalt_ui_data(self):
        ...
        ui_data['type'] = constant.PKG_TYPE_HEADER_STRAIGHT        
    ...
    def create_package_img(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs):
        labeled_image = inputs.addImageCommandInput('PinHeaderStraightImage', '', "Resources/img/Header-Male-Straight-Labeled.png")
    ...
    def create_dimension_ui(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs):
        ...
        terminal_post = inputs.addValueInput('terminalPostLength', 'L2', ao.units_manager.defaultLengthUnits, adsk.core.ValueInput.createByReal(self.ui_data['terminalPostLength']))
        terminal_post.tooltip = _LCLZ('terminalPostLength', 'Terminal Post Length')
        # table
...
PackageCommand.cmd_factory.register_command(constant.PKG_TYPE_HEADER_STRAIGHT, PackageCommandHeaderStraight) 
