...
class PackageCommandHeaderRightAngle(PackageCommandHeader.PackageCommandHeader):
    def __init__(self, name: str, options: dict):
        super().__init__(name, options)

        #overwrite some specific settings of this command. 
        self.cmd_id = 'cmd_id_header_right_angle'
        self.cmd_name = _LCLZ('CmdNameHeaderRightAngle', 'Pin Header(Male) Right Angle Generator')
        self.cmd_description = _LCLZ('CmdDescHeaderRightAngle', 'Generate Pin Header(Male) Right Angle Packages')
        self.cmd_ctrl_id = self.cmd_id
        self.dialog_width = 310
        self.dialog_height = 790
    ...
    def get_defalt_ui_data(self):
        ...
        ui_data['type'] = constant.PKG_TYPE_HEADER_RIGHT_ANGLE
        ui_data['bodyHeightMin'] = 0
        ui_data['bodyHeightMax'] = 0.556
        ui_data['bodyWidthMax'] = 0.254
        ui_data['bodyWidthMin'] = 0.254
    ...
    def create_package_img(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs):
        labeled_image = inputs.addImageCommandInput('PinHeaderRightAngleImage', '', "Resources/img/Header-Male-Right-Angle-Labeled.png")
    ...
    def create_dimension_ui(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs):
        ...
        terminal_post_length = inputs.addValueInput('terminalPostLength', 'L2', ao.units_manager.defaultLengthUnits, adsk.core.ValueInput.createByReal(self.ui_data['terminalPostLength']))
        terminal_post_length.tooltip = _LCLZ('terminalPostLength', 'Terminal Post Length')
        ver_tail_length = inputs.addValueInput('verticalTerminalTailLength', 'L3', ao.units_manager.defaultLengthUnits, adsk.core.ValueInput.createByReal(self.ui_data['verticalTerminalTailLength']))
...
PackageCommand.cmd_factory.register_command(constant.PKG_TYPE_HEADER_RIGHT_ANGLE, PackageCommandHeaderRightAngle) 
