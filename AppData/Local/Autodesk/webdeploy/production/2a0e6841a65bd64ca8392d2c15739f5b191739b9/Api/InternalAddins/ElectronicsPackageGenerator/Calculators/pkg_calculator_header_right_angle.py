...
# this class defines the package Calculator for the Header Right Angle Packages.
class PackageCalculatorHeaderRightAngle(pkg_calculator_header.PackageCalculatorHeader):
	...
	def get_ipc_3d_model_data(self):
		...
		model_data['L3'] = self.ui_data['verticalTerminalTailLength']
		return model_data
	...
	def get_ipc_package_description(self):
		family_name = 'Pin Header (Male) Right Angle'
		return super().get_ipc_package_description(family_name)

# register the calculator into the factory. 
pkg_calculator.calc_factory.register_calculator(constant.PKG_TYPE_HEADER_RIGHT_ANGLE, PackageCalculatorHeaderRightAngle) 