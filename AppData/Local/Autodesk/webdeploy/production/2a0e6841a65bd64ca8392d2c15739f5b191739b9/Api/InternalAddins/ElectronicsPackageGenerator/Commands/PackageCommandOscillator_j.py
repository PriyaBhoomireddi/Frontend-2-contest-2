...
class PackageCommandOscillator_j(PackageCommand.PackageCommand):
    def __init__(self, name: str, options: dict):
        super().__init__(name, options)

        #overwrite some specific settings of this command. 
        self.cmd_id = 'cmd_id_Oscillator_j'
        self.cmd_name = _LCLZ('CmdNameOscillator_j', 'Oscillator_j Generator')
        self.cmd_description = _LCLZ('CmdDescOscillator_j', 'Generate Oscillator_j Packages')
    ...
    def get_defalt_ui_data(self):
        ...
        ui_data['type'] = constant.PKG_TYPE_OSCILLATOR_J
        ...
        ui_data['bodyWidthMin'] = 0.84
        ui_data['bodyWidthMax'] = 0.88
        ui_data['bodyLengthMin'] = 1.38
        ui_data['bodyLengthMax'] = 1.42
        ui_data['bodyHeightMin'] = 0
        ui_data['bodyHeightMax'] = 0.47
        ui_data['bodyOffsetMin'] = 0.1
        ui_data['bodyOffsetMax'] = 0.1
        ui_data['customPadLength'] = 0.262
        ui_data['customPadWidth'] = 0.087
        ui_data['customPadSpan1'] = 1.052
    ...
    def on_create(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs):
        ...
        package_tab_cmd_inputs = inputs.addTabCommandInput('oscillator_jPackageTab', _LCLZ('package', 'Package'))
        oscillator_j_inputs = package_tab_cmd_inputs.children

        # Create image input.
        labeled_image = oscillator_j_inputs.addImageCommandInput('oscillator_jImage', '', "Resources/img/Oscillator-JLead-Labeled.png")
        labeled_image.isFullWidth = True
        labeled_image.isVisible = True

        pin_num = oscillator_j_inputs.addDropDownCommandInput('horizontalPadCount', _LCLZ('#Pins', '# Pins'), adsk.core.DropDownStyles.TextListDropDownStyle)
        ...
        pad_shape = oscillator_j_inputs.addDropDownCommandInput('padShape', _LCLZ('padShape', 'Pad Shape'), adsk.core.DropDownStyles.TextListDropDownStyle)
        ...
        rounded_corner = oscillator_j_inputs.addValueInput('roundedPadCornerSize', "Pad Roundness (%)", '', adsk.core.ValueInput.createByReal(self.ui_data['roundedPadCornerSize']))
        rounded_corner.isVisible = True if constant.SMD_PAD_SHAPE.get('RoundedRectangle') == self.ui_data['padShape'] else False

        pin_pitch = oscillator_j_inputs.addValueInput('verticalPinPitch', 'e', ao.units_manager.defaultLengthUnits, adsk.core.ValueInput.createByReal(self.ui_data['verticalPinPitch']))
        pin_pitch.tooltip = _LCLZ('pinPitch', 'Pin Pitch')
        terminal_centre = oscillator_j_inputs.addValueInput('terminalCenterToCenterDistance', 'E2', ao.units_manager.defaultLengthUnits, adsk.core.ValueInput.createByReal(self.ui_data['terminalCenterToCenterDistance']))
        terminal_centre.tooltip = _LCLZ('terminalCCGap', 'Terminal Center-Center Gap')

        # table
        table = oscillator_j_inputs.addTableCommandInput('bodyDimensionTable', 'Table', 3, '1:2:2')
        ...
        # Create dropdown input with test list style.
        map_silkscreen = oscillator_j_inputs.addDropDownCommandInput('silkscreenMappingTypeToBody', _LCLZ('mapSilkscreen', 'Map Silkscreen to Body'), adsk.core.DropDownStyles.TextListDropDownStyle)
        ...
        if not self.only_3d_model_generator:
            ...
            footprint_tab_cmd_inputs = inputs.addTabCommandInput('oscillator_jFootprintTab', _LCLZ('footprint', 'Footprint'))
            ...
            # Create image input.
            custom_footprint_image = custom_footprint_inputs.addImageCommandInput('customOscillator_jImage', '', "Resources/img/OscJL-Custom-Footprint.png")
...
PackageCommand.cmd_factory.register_command(constant.PKG_TYPE_OSCILLATOR_J, PackageCommandOscillator_j)
