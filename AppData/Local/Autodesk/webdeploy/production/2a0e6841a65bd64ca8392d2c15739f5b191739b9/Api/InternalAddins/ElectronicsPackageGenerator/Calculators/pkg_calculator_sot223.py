...
class PackageCalculatorSot223(pkg_calculator.PackageCalculator):
	...
	def get_footprint(self):
		...
		if self.ui_data['hasCustomFootprint'] :
			...
			odd_pin_pitch = self.ui_data['customPadPitch']
		else :
		...
		# build the odd pin on right side
		odd_pad = footprint.FootprintSmd(pin_pitch/2, 0, odd_pad_width, odd_pad_height)
		odd_pad.name = str(self.ui_data['horizontalPadCount'] )
		if self.ui_data['padShape']	== 'Rounded Rectangle':
		...
		right_intersect_y = odd_pad.center_point_y + odd_pad.height/2 + ipc_rules.SILKSCREEN_ATTRIBUTES['Clearance'] 
		#top side silkscreen
		if top_edge_y + stroke_width/2 > body_length/2:
			...
			line_top_right = footprint.FootprintWire(body_width/2, top_edge_y + stroke_width/2, body_width/2, right_intersect_y, stroke_width)
			...
			line_bottom_right = footprint.FootprintWire(body_width/2, -top_edge_y - stroke_width/2, body_width/2, - right_intersect_y, stroke_width)
			...
			footprint_data.append(line_top)
			line_top_right = footprint.FootprintWire(body_width/2, body_length/2, body_width/2, right_intersect_y, stroke_width)
			footprint_data.append(line_top_right)
			...
			footprint_data.append(line_bottom)
			line_bottom_right = footprint.FootprintWire(body_width/2, -body_length/2, body_width/2, - right_intersect_y, stroke_width)
			footprint_data.append(line_bottom_right)
	...
	def get_ipc_package_description(self):
		...
		short_description = str(self.ui_data['horizontalPadCount']) + '-SOT223, '
		...
		full_description = str(self.ui_data['horizontalPadCount']) + '-pin SOT223 package with '
	...
	def	get_ipc_package_metadata(self):
		...
		ao = addin_utility.AppObjects()
		self.metadata['ipcFamily'] = "SOT223"
...
pkg_calculator.calc_factory.register_calculator(constant.PKG_TYPE_SOT223, PackageCalculatorSot223) 