...
float4 PS_NPR(VS_TO_PS In) : SV_Target
{
	...
	if(gUseLines)
    {
        glow = gGlow.Sample(GlowSampler, texCoord);
    ...
    if(gUseHalo)
    {
        halo = gHalo.Sample(HaloSampler, texCoord);
        if(halo.a > 0.5){
			...
			{
				specialBlend( all, float4(halo.rgb, gHaloColor.a));
			}
        }
    }
