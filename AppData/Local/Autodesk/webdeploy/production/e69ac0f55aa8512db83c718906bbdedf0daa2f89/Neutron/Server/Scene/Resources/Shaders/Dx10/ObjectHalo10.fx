...
float4 CalcHalo(float4 id, float idz, float4 center, float centerz, float4 haloColorThis, float4 haloColor, inout float4 outColor)
{
...
float4 PS_NPR(VS_TO_PS In) : SV_Target
{
	...
	float4 depthValue = gDepth.Sample(DepthSampler, texCoord);
		...
		for ( int x = -stepRadius; x <= stepRadius; x++ )
		{
			...
			for ( int y = -stepRadius; y <= stepRadius; y++ )
			{
				...
				{	
					...
					float4 depthSample = gDepth.SampleLevel(DepthSampler,sampleCoord,0);
