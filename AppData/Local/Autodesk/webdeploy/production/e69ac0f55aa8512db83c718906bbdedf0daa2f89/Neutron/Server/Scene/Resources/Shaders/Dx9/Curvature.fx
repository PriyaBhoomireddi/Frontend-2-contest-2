...
sampler1D gDiffSamp : DiffTextureSampler = sampler_state
{
    Texture = <gDiffTex>;
};
...
VS_TO_PS curvatureVS(VS_INPUT IN)
{
	...
	OUT.CurData.x = IN.Curvature.x;
	OUT.CurData.y = IN.Curvature.y;
	if(gMapType == 0)
	{
		OUT.CurData.z = IN.Curvature.x * IN.Curvature.y;			
	}

    return OUT;
}

float4 curvaturePS(VS_TO_PS IN) : COLOR
{
	float x;
	...
	{
		float curvU = abs(IN.CurData.x);
		float curvV = abs(IN.CurData.y);
		if(gMapType == 1) {
			x = min(curvU, curvV);
		}
		else if(gMapType == 2) {
 	...
 	float3 color = tex1D(gDiffSamp, x).xyz;
...
technique Curvature
{
    pass P0
    {
        VertexShader = compile vs_2_0 curvatureVS();
        PixelShader = compile ps_2_0 curvaturePS();
    }
}
